...
if __name__ == '__main__': 
	...
	parser.add_argument('-out_dir', default='eval_e2e')
	...
	if not os.path.exists('eval_e2e'):
		os.mkdir(args.out_dir)
	if not os.path.exists('preview_e2e'):
		os.mkdir('preview_e2e')
	...
	with torch.no_grad():
		...
		# 	img_name = tp[1]
			...
			if args.evaluate == 1:
				...
				res_gt = '{0}/{1}'.format(args.images_dir, res_gt)
			...
			res_file = os.path.join(args.out_dir, 'res_{}.txt'.format(img_name.split('/')[-1])) ##########################
			res_file = codecs.open(res_file, 'w', 'utf-8')
...
#################################################################################################
				...
				if len(det_text) > 0:
					...
					for spl in splits_raw:
						...
						if len(spl[1][0]) >= eval_text_length:
							...
							res_file.write('{},{},{},{},{},{},{},{},1.0,{}\r\n'.format(int(boxw[0]), int(boxw[1]), int(boxw[2]), int(boxw[3]), int(boxw[4]), int(boxw[5]), int(boxw[6]), int(boxw[7]), spl[1][0]))
			...
			cv2.imwrite('preview_e2e/{0}'.format(base_nam), pix)
